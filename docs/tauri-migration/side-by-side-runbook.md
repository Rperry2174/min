...
- Loads `index.html` to `dist/bundle.js` in a standard Chromium window via `electron`.
...
The Tauri shell is a minimal migration target, not a full browser.  The address bar and navigation UI are intentionally stripped down.  Check the browser devtools (right-click, then Inspect) for console errors.
