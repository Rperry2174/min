...
| Credentials | `main/keychainService.js`, `js/passwordManager/keychain.js` | Blocked | `credential_store_*` commands exist but fail closed until backed by OS secret storage. **GAP:** Electron `safeStorage` OS-keychain encryption must be replaced with `tauri-plugin-stronghold` or the `keyring` crate. |
